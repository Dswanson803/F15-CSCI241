...
//Outputs the contact's information if an id is passed.
if (isset($_GET["id"]) && (!filter_var($_GET["id"], FILTER_VALIDATE_INT) === false)) {
	$currentId = $_GET["id"];
	echo "<h2>Contact Info: </h2><br />";
	echo "Name: " . $contacts[$currentId]["name"] . "<br />";
	echo "Phone: " . $contacts[$currentId]["phone"] . "<br />";
	echo "Email: " . $contacts[$currentId]["email"] . "<br />";
	echo "Address: " . $contacts[$currentId]["address"] . "<br />";
	echo "City: " . $contacts[$currentId]["city"] . "<br />";
	echo "State: " . $contacts[$currentId]["zip"] . "<br />";
	echo "Age: " . $contacts[$currentId]["age"] . "<br />";
	echo "<a href='contacts.php'>Return to Index.</a>";
} else { //Outputs the contact list if no id is passed.
	echo "<h1>All Contacts:</h1><br />";
	foreach ($contacts as $lineNo => $line) {
		echo "<a href=contacts.php?id=" . $lineNo . ">";
		echo $line['name'] . " - ";
		echo $line['city'] . " , ";
		echo $line['state'] . "<br />";
		echo "</a>";
